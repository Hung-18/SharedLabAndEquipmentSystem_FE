  ...
  template: `
    @if (open()) {
      <div class="fixed inset-0 z-[100] flex items-end justify-center p-0 sm:items-center sm:p-5">
        ...
        ></button>
        <section
          class="relative z-10 max-h-[94vh] w-full overflow-auto rounded-t-[30px] border border-white/60 bg-white shadow-2xl sm:max-w-2xl sm:rounded-[30px]"
          [style.max-width]="width()"
        >
          <header
            class="sticky top-0 z-10 flex items-start gap-4 border-b border-slate-100 bg-white/95 px-5 py-5 backdrop-blur sm:px-6"
          >
