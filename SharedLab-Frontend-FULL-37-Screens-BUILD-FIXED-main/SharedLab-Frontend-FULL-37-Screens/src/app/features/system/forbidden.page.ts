  ...
  template: `
    <main class="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#f5f7fb] p-5 text-center">
      ...
      <section class="relative w-full max-w-2xl rounded-[36px] border border-white bg-white/90 p-8 shadow-2xl shadow-slate-900/10 backdrop-blur-xl sm:p-14">
        ...
        <p class="mt-8 text-sm font-bold uppercase tracking-[0.22em] text-amber-600">Lỗi 403</p>
        <h1 class="mt-3 text-4xl font-bold tracking-[-0.04em] text-slate-950 sm:text-5xl">M không có quyền truy cập</h1>
        <p class="mx-auto mt-5 max-w-lg text-base leading-7 text-slate-500">Trang này chỉ dành cho một số vai trò nhất định. Hệ thống đã bảo vệ nội dung và không hiển thị bất kỳ thông tin kỹ thuật nào.</p>
        <div class="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
          <button type="button" class="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-[#111a3a] px-6 text-sm font-bold text-white shadow-lg shadow-slate-900/15 hover:bg-[#17234c]" (click)="goHome()">
            <app-icon name="home" [size]="18" />
            Về trang chủ
          </button>
          <button type="button" class="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-6 text-sm font-bold text-slate-700 hover:bg-slate-50" (click)="goBack()">
