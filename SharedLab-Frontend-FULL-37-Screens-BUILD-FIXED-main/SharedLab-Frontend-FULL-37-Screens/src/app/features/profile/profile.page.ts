        ...
        <div class="grid gap-6 xl:grid-cols-[360px_1fr]">
          <aside class="space-y-6">
            <article class="card-surface relative overflow-hidden p-6 text-center">
              <div class="absolute inset-x-0 top-0 h-28 bg-gradient-to-r from-[#111a3a] via-indigo-900 to-violet-800"></div>
              <div class="relative pt-8">
                <div class="mx-auto flex h-28 w-28 items-center justify-center rounded-[34px] border-[6px] border-white bg-gradient-to-br from-violet-400 to-indigo-600 text-3xl font-bold text-white shadow-xl shadow-indigo-950/20">
                  {{ initials(user.fullName) }}
                </div>
                <h2 class="mt-5 text-xl font-bold text-slate-950">{{ user.fullName }}</h2>
                <p class="mt-1 text-sm text-slate-400">&#64;{{ user.username }}</p>
                <span class="mt-4 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold" [class.bg-indigo-50]="user.roleName === 'Admin'" [class.text-indigo-700]="user.roleName === 'Admin'" [class.bg-cyan-50]="user.roleName === 'LabManager'" [class.text-cyan-700]="user.roleName === 'LabManager'" [class.bg-violet-50]="user.roleName === 'Requester'" [class.text-violet-700]="user.roleName === 'Requester'">
              ...
              <div class="mt-6 grid grid-cols-2 gap-3 border-t border-slate-100 pt-6">
                ...
                <div class="rounded-2xl bg-slate-50 p-4">
                  <p class="text-sm font-bold" [class.text-emerald-600]="statusText() === 'Active'" [class.text-amber-600]="statusText() === 'Restricted'" [class.text-rose-600]="statusText() === 'Locked' || statusText() === 'Inactive'">{{ statusLabel(statusText()) }}</p>
